...
`
  ...
  const { rangeStart, rangeEnd } = getYearRange(startYear, endYear)
  const [year, setYear] = useState(rangeStart)
  ...
  const handleDayChange = selectedDay => {
    setDay(selectedDay)
    onChange(`${selectedDay} ${monthNames[month]} ${year}`)
    setDateDropdown(false)
  }

  const handleMonthChange = direction => {
    setMonth(prev => (prev + direction + 12) % 12)
    if (direction === -1 && month === 0) setYear(prev => prev - 1)
    if (direction === 1 && month === 11) setYear(prev => prev + 1)
  }
  ...
  return (
    ...
    >
      <div
        onClick={() => setDateDropdown(prev => !prev)}
        style={{
          display: 'flex',
